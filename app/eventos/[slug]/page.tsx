...
import DollarEventClient from './DollarEventClient';
import { Metadata } from 'next';
import { 
  Trophy, Calendar, Clock, Users, TrendingUp, Package, Utensils, 
  Wrench, Info, BarChart3, ChartBar, Target, DollarSign, Edit3, Eye 
} from 'lucide-react';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const eventTypeText = event.eventType === 'DOLLAR_PREDICTION' 
    ? 'Predicción del Dólar MEP' 
    : 'Predicción del IPC';

  return {
    title: `${event.name} - Evento de ${eventTypeText} | ArgenStats`,
    description: event.description || `Participa en ${event.name} y gana ${event.prizeCurrency} ${event.prizeAmount}`,
    openGraph: {
      title: event.name,
      description: event.description || `Evento de predicción económica con premio de ${event.prizeCurrency} ${event.prizeAmount}`,
      type: 'website',
      url: `https://argenstats.com/eventos/${slug}`,
      images: [
        {
          url: '/og-event.png',
          width: 1200,
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="group bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-5 border border-blue-200 dark:border-blue-800 hover:scale-105 transition-transform">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-medium text-blue-700 dark:text-blue-400">PUBLICACIÓN</span>
                    </div>
              ...
              <div className="group bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 rounded-xl p-5 border border-red-200 dark:border-red-800 hover:scale-105 transition-transform">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      {statistics?.averagePredictions?.ipcGeneral?.toFixed(2) || '0.00'}%
                    </p>
                  </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <section className="relative bg-gradient-to-br from-green-50 to-emerald-100 dark:from-gray-800 dark:to-gray-900 overflow-hidden">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-40 -right-40 w-80 h-80 bg-gradient-to-br from-green-200 to-emerald-200 dark:from-green-900/30 dark:to-emerald-900/30 rounded-full blur-3xl opacity-60 dark:opacity-40" />
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="group bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-xl p-5 border border-blue-200 dark:border-blue-800 hover:scale-105 transition-transform">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-medium text-blue-700 dark:text-blue-400">FECHA EVENTO</span>
                    </div>
                    <p className="text-lg font-semibold text-gray-900 dark:text-white">
                      {formatDate(event.eventDate)}
                    </p>
              ...
              <div className="group bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl p-5 border border-green-200 dark:border-green-800 hover:scale-105 transition-transform">
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-xs font-medium text-green-700 dark:text-green-400">PROMEDIO</span>
                    </div>
                    <p className="text-xl font-bold text-gray-900 dark:text-white">
                      ${statistics?.averagePrediction || '0.00'}
                    </p>
                  </div>
...
export default async function EventPage({ params }: Props) {
  ...
  // Preparar datos del usuario para los componentes cliente
  const userData = user ? {
    id: user.id,
  ...
  if (event.eventType === 'DOLLAR_PREDICTION') {
    // Para eventos de Dólar
    let userPrediction = null;
    const statistics = await DollarEventsService.getEventStatistics(event.id);
    
    if (userId) {
      const allPredictions = await DollarEventsService.getAllPredictions(event.id);
      userPrediction = allPredictions.find(p => p.userId === userId) || null;
    }
