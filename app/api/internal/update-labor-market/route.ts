...
export async function POST(request: NextRequest) {
  try {
    ...
    console.info(`📊 Obtenidos ${laborData.length} registros del mercado laboral`)
    ...
    const breakdown = {
      ...
      demographic: laborData.filter(item => item.dataType === 'demographic').length
    }
    ...
    return NextResponse.json(
      { error: 'Failed to update labor market', details: (error as Error).message },
      { status: 500 }
    )
  }
}