...
const InflationCalculator = memo(function InflationCalculator({ initialCERData }: InflationCalculatorProps) {
  ...
  // Función para obtener CER actual (similar a fetchAllRates del conversor)
  const fetchCurrentCER = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Error al obtener CER actual');
  ...
  // Función para calcular inflación usando la API
  const calculateInflationAPI = useCallback(async () => {
    ...
    try {
      ...
      // Llamada directa a la API del CER con view=calculator
      const response = await fetch(
        `/api/v1/cer?view=calculator&amount=${numAmount}&from=${fromStr}&to=${toStr}`
      );
        ...
        {currentCER && (
          <div className="mb-4">
            <div className="flex items-center justify-between p-3 bg-orange-50 border border-orange-200 rounded-lg">
              <span className="text-sm text-orange-600">Índice CER actual:</span>
              <div className="flex items-center gap-4">
                <span className="font-bold text-orange-700">{currentCER.value.toFixed(4)}</span>
                {currentCER.yearly_pct_change && (
                  <Badge variant="outline" className="border-orange-300 text-orange-700">
                    +{currentCER.yearly_pct_change.toFixed(2)}% anual
                  </Badge>
                )}
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 p-4 border border-gray-300 rounded-xl focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-500 focus-within:ring-opacity-20">
                ...
                <div className="text-lg font-bold text-gray-600 shrink-0">ARS</div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3 p-4 bg-gray-50 border border-gray-200 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-lg font-bold text-gray-900 truncate">
                    {result ? 
                      (result.inverted ? result.initial_amount : result.adjusted_amount).toLocaleString('es-AR', { 
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-3">
                <p className="text-sm text-orange-800 leading-relaxed">
                  {calculationDirection === 'PAST_TO_PRESENT' 
                    ? `$${parseFloat(amount).toLocaleString('es-AR')} del ${fromDate.toLocaleDateString('es-AR')} equivalen hoy a $${result.adjusted_amount?.toLocaleString('es-AR', { minimumFractionDigits: 2 })}`
                    : `Para tener el poder de compra de $${parseFloat(amount).toLocaleString('es-AR')} de hoy, necesitabas $${result.initial_amount?.toLocaleString('es-AR', { minimumFractionDigits: 2 })} el ${fromDate.toLocaleDateString('es-AR')}`
                  }
                </p>
